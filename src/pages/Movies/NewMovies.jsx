      ...
      {movies.map((movie)=>(
        <div key={movie.movie_id} className="cursor-pointer" onClick={() => { setSelectedMovie(movie); setViewmovie(true); }}>
           <img
                ...
                } alt={movie.movie_name} className="w-45 h-60 rounded-[10px]"
                loading='lazy' />
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 " >
      ...
      <div className='movie-details  bg-white w-full max-w-3xl md:w-[80%] p-4 rounded-lg relative z-10 ' style={{padding:'10px'}}>
        ...
        <p>{selectedMovie.movie_description || selectedMovie.description || 'No description available.'}</p><br />
          <i className="fa-solid fa-trash text-red-600 cursor-pointer absolute right-5 bottom-2 text-[20px] cursor-pointer" title='delete' onClick={() => deletMovie(selectedMovie.movie_id)}></i>
      </div>
    </div>
    )}
